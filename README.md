...
자동급수는 식물별로 켜고 끄는 구조입니다. 모든 식물이 자동급수 대상이 되는 것이 아니라, 자동급수 대상으로 지정한 식물만 펌프 명령 생성 대상이 됩니다.

켜고 끄기, 토양센서 지정, 수분 기준·급수 초·쿨다운·하루 최대 횟수는 모두 **'급수' 탭**에서 다룹니다. 위쪽 폼으로 전체 식물에 같은 값을 한 번에 넣고, 아래 표에서 식물별로 따로 고칩니다.
